...
class APIClient {
  ...
  private async fetch<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    ...
    const url = endpoint.startsWith("/v3") ? `${this.baseUrl}${endpoint}` : `${this.baseUrl}/v3${endpoint}`;
  ...
  async createUser(data: UserCreateRequest): Promise<UserCreateResponse> {
    return this.fetch<UserCreateResponse>("/users", {
      method: "POST",
  ...
  async createTeacher(firstName: string, lastName: string): Promise<UserCreateResponse> {
  ...
  async getJoinLink(assignmentId: number, studentId: string, target: AssignmentJoinTarget = "web"): Promise<AssignmentJoinResponse> {
    console.log("Getting join link for:", { assignmentId, studentId, target });
    const payload = {
  ...
  async launchStandard(standardId: number | string, studentIds: string[], target: AssignmentJoinTarget = "web"): Promise<LaunchResponse> {
    console.log("Launching standard:", { standardId, studentIds, target });
    ...
    const joinPromises = studentIds.map(async (studentId) => {
      const joinResponse = await this.getJoinLink(assignment.assignment_id, studentId, target);
      console.log("Join link created for student:", {
        studentId,
        joinResponse,
      });
      return {
